...
### Phase 1 — 레이아웃 & 기본 구조
- [ ] `app/layout.tsx` — 폰트, 메타데이터 설정
- [ ] `app/page.tsx` — 섹션 조합 진입점
- [ ] `components/` 폴더 구조 잡기
- [ ] 다크모드 토글 (Tailwind `dark:` 클래스 기반)

### Phase 2 — 섹션 컴포넌트 개발
- [ ] `HeroSection` — 이름 + 타이핑 효과 + CTA 버튼
- [ ] `AboutSection` — 텍스트 + 간단한 프로필
- [ ] `ProjectsSection` — 카드 그리드 (프로젝트별 스크린샷, 설명, 링크)
- [ ] `TechStackSection` — 기술 뱃지 나열
- [ ] `ContactSection` — 링크 버튼들

### Phase 3 — 데이터 분리
- [ ] `data/projects.ts` — 프로젝트 데이터 상수로 분리
- [ ] `data/techStack.ts` — 기술 스택 데이터 분리
